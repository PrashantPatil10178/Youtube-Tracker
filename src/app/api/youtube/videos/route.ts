...
/**
 * GET /api/youtube/videos?std=<9|10|11|12|all>
 *
 * Thin wrapper over `computeWorkspaceVideos` — server components call that
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(await computeWorkspaceVideos(channels, std, workspaceId), {
      headers: { 'Cache-Control': 'public, s-maxage=600, stale-while-revalidate=3600' }
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json({ error: `Workspace videos failed: ${message}` }, { status: 502 });
  }
}
